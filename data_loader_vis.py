...
class SYSUData(data.Dataset):
    def __init__(self, data_dir, transform=None, colorIndex=None, thermalIndex=None):
        ...
        self.transform_post = transforms.Compose([normalize,])

    def __getitem__(self, index):
        img1, target1 = self.train_color_image[self.cIndex[index]], self.train_color_label[self.cIndex[index]]
        img2, target2 = self.train_thermal_image[self.tIndex[index]], self.train_thermal_label[self.tIndex[index]]

        img11 = self.transform_color(img1)
        img22 = self.transform_thermal(img2)

        img1_norm = self.transform_post(img11)
        img2_norm = self.transform_post(img22)
        return img1, img2, img1_norm, img2_norm, target1, target2
...
class TestData(data.Dataset):
    def __init__(self, test_img_file, test_label, transform=None, img_size=(144, 288)):
        ...
        self.transform = transform
        self.embed_fn, self.input_ch = get_embedder(4, 0)
