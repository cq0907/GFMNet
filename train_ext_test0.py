...
from model_test0 import embed_net
from utils import *
from loss import OriTripletLoss, TripletLoss_WRT, TripletLoss_ADP0
...
parser.add_argument('--model_path', default='save_model0/', type=str,
                    help='model save path')
parser.add_argument('--save_epoch', default=20, type=int,
                    metavar='s', help='save model every 10 epochs')
parser.add_argument('--log_path', default='log0/', type=str,
                    help='log save path')
...
suffix = suffix + '_p{}_n{}_lr_{}_seed_{}_9'.format(args.num_pos, args.batch_size, args.lr, args.seed)
...
if args.method == 'base':
    net = embed_net(n_class, no_local='off', gm_pool='off', arch=args.arch, share_net=args.share_net)
else:
...
if args.method == 'agw':
    ...
    criterion_tri = TripletLoss_ADP0(alpha=args.alpha, gamma=args.gamma, square=args.square)
else:
    loader_batch = args.batch_size * args.num_pos
    criterion_tri = OriTripletLoss(batch_size=loader_batch, margin=args.margin)
...
def train(epoch):
    ...
    for batch_idx, (input10, input11, input2, label1, label2) in enumerate(trainloader):
        ...
        feat, out0, lf = net(input1, input2)

        loss_id = criterion_id(out0, labels)
        loss_tri, loss_local_tri, batch_acc = criterion_tri(feat, labels, lf)
        ...
        # loss = loss_id + loss_tri + loss_js + loss_circle
        loss = loss_id + loss_tri + loss_local_tri * 0.5
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        tri_local_loss.update(loss_local_tri.item(), 2 * input1.size(0))
        total += labels.size(0)
        ...
        if batch_idx % 50 == 0:
            print('Epoch: [{}][{}/{}] '
                  ...
                  'Local: {tri_local_loss.val:.4f} ({tri_local_loss.avg:.4f}) '
                  'Accu: {:.2f}'.format(
                epoch, batch_idx, len(trainloader), current_lr,
                100. * correct / total, batch_time=batch_time,
                train_loss=train_loss, id_loss=id_loss, tri_loss=tri_loss,tri_local_loss=tri_local_loss
            ))
...
for epoch in range(start_epoch, 101):
    ...
    sampler = IdentitySampler(trainset.train_color_label,
                              ...
                              color_pos, thermal_pos, args.num_pos,
