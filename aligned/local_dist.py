...
def shortest_dist1(dist_mat, dist_an):
    """Parallel version.
    Args:
      dist_mat: pytorch Variable, available shape:
        1) [m, n]
    ...
    """
    b, m, n = dist_mat.size()
    dist = torch.zeros_like(dist_an)
    for i in range(b):
        M = dist_mat[i]
        total_s, j, k, stride = M[0][0], 0, 0, 0
        while True:
            if j == m-1 and k == n-1:
                break
            else:
                stride += 1
                row_index = torch.tensor(float('inf')) if k == m-1 else torch.argmin(M[j, k+1:], dim=0)
                col_index = torch.tensor(float('inf')) if j == n-1 else torch.argmin(M[j+1:, k], dim=0)
                if torch.isinf(row_index) or torch.isinf(col_index):
                    if torch.isinf(row_index):
                        total_s += M[j + 1 + col_index, k]
                        j = j + 1 + col_index
                    elif torch.isinf(col_index):
                        total_s += M[j, k + 1 + row_index]
                        k = k + 1 + row_index
                    continue
                if M[j, k+1+row_index] > M[j+1+col_index, k]:
                    total_s += M[j+1+col_index, k]
                    j = j+1+col_index
                else:
                    total_s += M[j, k+1+row_index]
                    k = k+1+row_index
        dist[i] = total_s/stride
    return dist
...
def batch_local_dist(x, y):
    ...
    # m, n = dist_mat.shape[1:]
    # dist_mat = (dist_mat / torch.sum(torch.sum(dist_mat, dim=-1), dim=-1, keepdim=True).unsqueeze(-1)) * (m*n)
    # dist_mat = (torch.exp(torch.pow(dist_mat, 2)) - 1.) / (torch.exp(torch.pow(dist_mat, 2)) + 1.)
    # shape [N]
    dist = shortest_dist(dist_mat.permute(1, 2, 0))
    return dist

def batch_local_dist1(x, y, dist_an):
    """
    Args:
      x: pytorch Variable, with shape [N, m, d]
      y: pytorch Variable, with shape [N, n, d]
    ...
    # dist_mat = (torch.exp(torch.pow(dist_mat, 2)) - 1.) / (torch.exp(torch.pow(dist_mat, 2)) + 1.)
    # shape [N]
    # dist = shortest_dist1(dist_mat.permute(1, 2, 0))
    dist = shortest_dist1(dist_mat, dist_an)
    return dist
