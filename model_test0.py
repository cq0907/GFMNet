...
class AFAM(nn.Module):
    ...
    def forward(self, x):
        ...
        # out = self.channel_attention(x) * self.non_local(x)
        # out = self.spatial_attention(out) * self.non_local(out)
...
class HorizontalMaxPool2d(nn.Module):
    ...
    def forward(self, x):
        inp_size = x.size()
        return nn.functional.max_pool2d(input=x,kernel_size= (3, inp_size[3]))
...
class embed_net(nn.Module):
    def __init__(self,  class_num, no_local= 'on', gm_pool = 'on', arch='resnet50', share_net=1, aligned=True):
    ...
    def forward(self, x1, x2, modal=0):
        ...
        if self.training:
            return x_pool, self.classifier(feat), lf
